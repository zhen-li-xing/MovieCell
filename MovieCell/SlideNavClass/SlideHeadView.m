...
-(void)setUpOneChildController:(NSInteger)index{
    ...
    NSLog(@"%@",vc.title);
    NSLog(@"%lu",(unsigned long)superVC.childViewControllers.count);
    
    //通知传值
    //1.创建一个通知
    NSNotification * notice = [[NSNotification alloc] initWithName:@"scrollViewTitle" object:nil userInfo:@{@"title":vc.title}];
    //2.发送通知
